...
// alef:hash:23686456793b6dd80bf87a1737507ff9acc0019845d7a2e5d38d89148402ab7e
//
...
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_io_xberg_android_HtmlToMarkdownRsBridge_nativeHeaderMetadataIsValid(
    mut env: EnvUnowned,
    _class: JClass,
    self_json: JString,
) -> jboolean {
    ...
    let Some(__jni_result) = run_or_throw(env, |env| {
        ...
        let mut client: core_crate::HeaderMetadata = match serde_json::from_str(&self_str) {
            Ok(v) => v,
            Err(e) => {
                throw_jni_error(env, &format!("HeaderMetadata deserialize: {e}"));
                return false;
            }
        };
        let v = client.is_valid();
        v
    }) else {
        return false;
    };
