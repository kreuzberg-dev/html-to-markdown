...
// alef:hash:feda186ccbd707b9b6529effc2c48d36991073d5f52a94bd876cd36aa8ff2761
...
fn export_generated_headers() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut header = String::from_utf8(bytes).map_err(|error| {
        io::Error::new(
            ...
            format!("cbindgen generated invalid UTF-8 for include/html_to_markdown.h: {error}"),
        )
    })?;

    stamp_enabled_features(&mut header);
    let destinations = [
        Path::new("include/html_to_markdown.h"),
        Path::new("../../packages/go/include/html_to_markdown.h"),
    ];
