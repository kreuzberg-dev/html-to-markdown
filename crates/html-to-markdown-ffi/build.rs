...
// alef:hash:f4e136f05a8e14d6eb58cb1b56fcbe4522073a884e09b38ae88e74812fb6c213
...
fn main() {
    ...
    // `#if defined(HTM_FEATURE_X)` (its `[defines]` table), and it does so from
    ...
    {
        let header_path = "include/html_to_markdown.h";
        ...
        if !header.contains(marker) && header.contains("HTM_FEATURE_") {
            ...
            let mut defines: Vec<String> = std::env::vars_os()
                .filter_map(|(key, _)| {
                    ...
                    Some(format!("#define HTM_FEATURE_{name} 1"))
                })
                .collect();
            defines.sort();
            defines.dedup();
    ...
    // `binding.go` carries `#include "html_to_markdown.h"` under cgo, so the header has to be
