...
<!-- alef:hash:6d2e5c921c6929e66532eeb9f79f90f5386737c78195bddcc75554e01f81ec13 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## C API Reference <span class="version-badge">v3.11.1</span>
...
|-------|------|---------|-------------|
| `heading_style` | `HTMAlefHandle` | `HTM_HTM_ATX` | Heading style to use in Markdown output (ATX `#` or Setext underline). |
| `list_indent_type` | `HTMAlefHandle` | `HTM_HTM_SPACES` | How to indent nested list items (spaces or tab). |
...
| `highlight_style` | `HTMAlefHandle` | `HTM_HTM_DOUBLE_EQUAL` | Style used for `<mark>` / highlighted text (e.g. `==text==`). |
...
| `newline_style` | `HTMAlefHandle` | `HTM_HTM_SPACES` | How to encode hard line breaks (`<br>`) in Markdown. |
| `code_block_style` | `HTMAlefHandle` | `HTM_HTM_BACKTICKS` | Style used for fenced code blocks (backticks or tilde). |
...
| `link_style` | `HTMAlefHandle` | `HTM_HTM_INLINE` | Link rendering style (inline or reference). |
| `output_format` | `HTMAlefHandle` | `HTM_HTM_MARKDOWN` | Target output format (Markdown, plain text, etc.). |
...
| `tier_strategy` | `HTMAlefHandle` | `HTM_HTM_AUTO` | Which conversion tier to use. - `TierStrategy.Auto` (default) — automatically choose the best path. - `TierStrategy.Tier2` — always use the Tier-2 DOM-walk path. - `TierStrategy.Tier1` — always attempt Tier-1 (testkit only). |
...
| `tables` | `HTMAlefHandle*` | `NULL` | Extracted tables with structured cell data and markdown representation. Table data is collected by the same pass that builds `Self.document`, so it is populated only when the `include_document_structure` option is `true`. With the default options this is an empty list even for input that contains tables — the tables still appear in `Self.content` as rendered Markdown. |
| `warnings` | `HTMAlefHandle*` | `NULL` | Non-fatal processing warnings. |
...
| `annotations` | `HTMAlefHandle*` | `/* serde(default) */` | Inline formatting annotations (bold, italic, links, etc.) with byte offsets into the text. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `nodes` | `HTMAlefHandle*` | — | All nodes in document reading order. |
...
| `row_span` | `uint32_t` | `default_span()` | Number of rows this cell spans (default 1). |
| `col_span` | `uint32_t` | `default_span()` | Number of columns this cell spans (default 1). |
...
| `headers` | `HTMAlefHandle*` | `NULL` | Extracted header elements with hierarchy |
| `links` | `HTMAlefHandle*` | `NULL` | Extracted hyperlinks with type classification |
| `images` | `HTMAlefHandle*` | `NULL` | Extracted images with source and dimensions |
| `structured_data` | `HTMAlefHandle*` | `NULL` | Extracted structured data blocks |
...
| `preset` | `HTMAlefHandle` | `HTM_HTM_STANDARD` | Preprocessing preset level (Minimal, Standard, Aggressive) |
...
| `cells` | `HTMAlefHandle*` | `NULL` | All cells in the table as a flat, sparse list. The list is ordered by `(row, col)` but is **not** a dense `rows × cols` matrix: cells that are covered by a spanning cell (via `row_span > 1` or `col_span > 1`) do not appear in the list. Only the top-left "origin" cell of a span is present, with its `row_span` and `col_span` fields set accordingly. To reconstruct the full visual grid, iterate over all cells and mark the rectangular region `[row .. row+row_span, col .. col+col_span]` as occupied by that cell. Any `(row, col)` position that is not the origin of any cell is covered by a span from an earlier cell. The length of this list is `≤ rows * cols`. An empty table (`rows == 0 \|\| cols == 0`) produces an empty list. |
...
| `HTM_METADATA_BLOCK` | A block of key-value metadata pairs (from `<head>` meta tags). — Fields: `entries`: `HTMAlefHandle*` |
