...
<!-- alef:hash:69e1b8778574af690375e19d364d0841eb8c7d4cb50b70efe1603a88910f5614 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Zig API Reference <span class="version-badge">v3.11.4</span>
...
```zig
pub fn convert(html: [:0]const u8, options: ?ConversionOptions) ConversionError!ConversionResult
```
...
| `strong_em_symbol` | `\[:0\]const u8` | `"*"` | Character used for bold/italic emphasis markers (`*` or `_`). |
...
| `code_language` | `\[:0\]const u8` | `""` | Default language annotation for fenced code blocks that have no language hint. |
...
| `sub_symbol` | `\[:0\]const u8` | `""` | Markdown notation for subscript text (e.g. `"~"`). |
| `sup_symbol` | `\[:0\]const u8` | `""` | Markdown notation for superscript text (e.g. `"^"`). |
...
| `keep_inline_images_in` | `\[\]const \[:0\]const u8` | `[]` | HTML tag names whose `<img>` children are kept inline instead of block. |
...
| `strip_tags` | `\[\]const \[:0\]const u8` | `[]` | HTML tag names whose content is stripped from the output entirely. |
| `preserve_tags` | `\[\]const \[:0\]const u8` | `[]` | HTML tag names that are preserved verbatim in the output. |
...
| `canonical_url` | `\[:0\]const u8?` | `null` | Canonical URL from `<link rel="canonical">` tag |
| `base_href` | `\[:0\]const u8?` | `null` | Base URL from `<base href="">` tag for resolving relative URLs |
...
| `open_graph` | `std.StringHashMap(\[:0\]const u8)` | `{}` | Open Graph metadata (og:* properties) for social media Keys like "title", "description", "image", "url", etc. |
| `twitter_card` | `std.StringHashMap(\[:0\]const u8)` | `{}` | Twitter Card metadata (twitter:* properties) Keys like "card", "site", "creator", "title", "description", "image", etc. |
| `meta_tags` | `std.StringHashMap(\[:0\]const u8)` | `{}` | Additional meta tags not covered by specific fields Keys are meta name/property attributes, values are content |
...
| `source_format` | `\[:0\]const u8?` | `null` | The source format (always "html" for this library). |
...
| `row_span` | `u32` | `default_span()` | Number of rows this cell spans (default 1). |
| `col_span` | `u32` | `default_span()` | Number of columns this cell spans (default 1). |
...
```zig
pub fn visit_text(self: *const HtmlVisitor, ctx: NodeContext, text: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_element_start(self: *const HtmlVisitor, ctx: NodeContext) VisitResult
```
...
```zig
pub fn visit_element_end(self: *const HtmlVisitor, ctx: NodeContext, output: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_link(self: *const HtmlVisitor, ctx: NodeContext, href: [:0]const u8, text: [:0]const u8, title: ?[:0]const u8) VisitResult
```
...
```zig
pub fn visit_image(self: *const HtmlVisitor, ctx: NodeContext, src: [:0]const u8, alt: [:0]const u8, title: ?[:0]const u8) VisitResult
```
...
```zig
pub fn visit_heading(self: *const HtmlVisitor, ctx: NodeContext, level: u32, text: [:0]const u8, id: ?[:0]const u8) VisitResult
```
...
```zig
pub fn visit_code_block(self: *const HtmlVisitor, ctx: NodeContext, lang: ?[:0]const u8, code: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_code_inline(self: *const HtmlVisitor, ctx: NodeContext, code: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_list_item(self: *const HtmlVisitor, ctx: NodeContext, ordered: bool, marker: [:0]const u8, text: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_list_start(self: *const HtmlVisitor, ctx: NodeContext, ordered: bool) VisitResult
```
...
```zig
pub fn visit_list_end(self: *const HtmlVisitor, ctx: NodeContext, ordered: bool, output: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_table_start(self: *const HtmlVisitor, ctx: NodeContext) VisitResult
```
...
```zig
pub fn visit_table_row(self: *const HtmlVisitor, ctx: NodeContext, cells: []const [:0]const u8, is_header: bool) VisitResult
```
...
```zig
pub fn visit_table_end(self: *const HtmlVisitor, ctx: NodeContext, output: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_blockquote(self: *const HtmlVisitor, ctx: NodeContext, content: [:0]const u8, depth: u64) VisitResult
```
...
```zig
pub fn visit_strong(self: *const HtmlVisitor, ctx: NodeContext, text: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_emphasis(self: *const HtmlVisitor, ctx: NodeContext, text: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_strikethrough(self: *const HtmlVisitor, ctx: NodeContext, text: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_underline(self: *const HtmlVisitor, ctx: NodeContext, text: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_subscript(self: *const HtmlVisitor, ctx: NodeContext, text: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_superscript(self: *const HtmlVisitor, ctx: NodeContext, text: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_mark(self: *const HtmlVisitor, ctx: NodeContext, text: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_line_break(self: *const HtmlVisitor, ctx: NodeContext) VisitResult
```
...
```zig
pub fn visit_horizontal_rule(self: *const HtmlVisitor, ctx: NodeContext) VisitResult
```
...
```zig
pub fn visit_custom_element(self: *const HtmlVisitor, ctx: NodeContext, tag_name: [:0]const u8, html: [:0]const u8) VisitResult
```
...
|------|------|----------|-------------|
| `ctx` | `NodeContext` | Yes | The node context |
| `tag_name` | `\[:0\]const u8` | Yes | The  tag name |
| `html` | `\[:0\]const u8` | Yes | The  html |
...
```zig
pub fn visit_definition_list_start(self: *const HtmlVisitor, ctx: NodeContext) VisitResult
```
...
```zig
pub fn visit_definition_term(self: *const HtmlVisitor, ctx: NodeContext, text: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_definition_description(self: *const HtmlVisitor, ctx: NodeContext, text: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_definition_list_end(self: *const HtmlVisitor, ctx: NodeContext, output: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_form(self: *const HtmlVisitor, ctx: NodeContext, action: ?[:0]const u8, method: ?[:0]const u8) VisitResult
```
...
```zig
pub fn visit_input(self: *const HtmlVisitor, ctx: NodeContext, input_type: [:0]const u8, name: ?[:0]const u8, value: ?[:0]const u8) VisitResult
```
...
|------|------|----------|-------------|
| `ctx` | `NodeContext` | Yes | The node context |
| `input_type` | `\[:0\]const u8` | Yes | The  input type |
...
```zig
pub fn visit_button(self: *const HtmlVisitor, ctx: NodeContext, text: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_audio(self: *const HtmlVisitor, ctx: NodeContext, src: ?[:0]const u8) VisitResult
```
...
```zig
pub fn visit_video(self: *const HtmlVisitor, ctx: NodeContext, src: ?[:0]const u8) VisitResult
```
...
```zig
pub fn visit_iframe(self: *const HtmlVisitor, ctx: NodeContext, src: ?[:0]const u8) VisitResult
```
...
```zig
pub fn visit_details(self: *const HtmlVisitor, ctx: NodeContext, open: bool) VisitResult
```
...
```zig
pub fn visit_summary(self: *const HtmlVisitor, ctx: NodeContext, text: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_figure_start(self: *const HtmlVisitor, ctx: NodeContext) VisitResult
```
...
```zig
pub fn visit_figcaption(self: *const HtmlVisitor, ctx: NodeContext, text: [:0]const u8) VisitResult
```
...
```zig
pub fn visit_figure_end(self: *const HtmlVisitor, ctx: NodeContext, output: [:0]const u8) VisitResult
```
...
| `tag_name` | `\[:0\]const u8` | — | Raw HTML tag name (e.g., "div", "h1", "custom-element") |
| `depth` | `u64` | — | Depth in the DOM tree (0 = root) |
| `index_in_parent` | `u64` | — | Index among siblings (0-based) |
| `parent_tag` | `\[:0\]const u8?` | `null` | Parent element's tag name (None if root) |
...
| `raw_json` | `\[:0\]const u8` | — | Raw JSON string (for JSON-LD) or serialized representation |
| `schema_type` | `\[:0\]const u8?` | `null` | Schema type if detectable (e.g., "Article", "Event", "Product") |
...
| `list_item` | A single list item. — Fields: `text`: `[:0]const u8` |
| `table` | A table with structured cell data. — Fields: `grid`: `TableGrid` |
| `image` | An image element. — Fields: `description`: `[:0]const u8`, `src`: `[:0]const u8`, `image_index`: `u32` |
...
| `definition_item` | A definition list entry with term and description. — Fields: `term`: `[:0]const u8`, `definition`: `[:0]const u8` |
| `raw_block` | A raw block preserved as-is (e.g. `<script>`, `<style>` content). — Fields: `format`: `[:0]const u8`, `content`: `[:0]const u8` |
| `metadata_block` | A block of key-value metadata pairs (from `<head>` meta tags). — Fields: `entries`: `[]const MetadataEntry` |
| `group` | A section grouping container (auto-generated from heading hierarchy). — Fields: `label`: `[:0]const u8`, `heading_level`: `u8`, `heading_text`: `[:0]const u8` |
