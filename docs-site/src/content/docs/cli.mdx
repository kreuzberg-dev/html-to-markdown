...
Or on Windows via [Scoop](https://scoop.sh):
...
| `--url-escape-style STYLE` | `angle`, `percent` | `angle` | `angle` wraps destinations containing spaces or newlines in `<...>`. `percent` percent-encodes every character that is not RFC 3986 unreserved or `/`. |
...
| `--max-image-size BYTES`           | Skip inline images whose decoded payload exceeds this size. Default `5242880` (5 MB). Requires `--extract-inline-images`. |
| `--capture-svg`                    | Capture inline `<svg>` elements as extracted images. Requires `--extract-inline-images`. |
| `--no-infer-dimensions`            | Skip inferring image width and height from the decoded payload when the HTML omits them. Requires `--extract-inline-images`. |
...
| `--exclude-selectors SELECTORS` | Comma-separated CSS selectors. Matching elements and their descendants are removed before conversion (e.g. `.ad,#sidebar`). |
...
| Flag                     | Values          | Default | Description                                                                                          |
| ------------------------ | --------------- | ------- | ---------------------------------------------------------------------------------------------------- |
| `--tier-strategy STRATEGY` | `auto`, `tier2` | `auto`  | `auto` picks the fastest conversion path the input allows. `tier2` always takes the DOM-walk path. Output is identical either way; this exists for benchmarking and bug isolation. |
...
| `--url-escape-style STYLE`      | `url_escape_style`           | `"angle"` \| `"percent"`                                   |
| `--keep-inline-images-in ELEMS` | `keep_inline_images_in`      | comma-separated tag list                                   |
| `--skip-images`                 | `skip_images`                | boolean flag                                               |
| `--max-image-size BYTES`        | `max_image_size`             | integer; default `5242880`                                 |
| `--capture-svg`                 | `capture_svg`                | boolean flag                                               |
| `--no-infer-dimensions`         | `infer_dimensions`           | inverted: flag sets `infer_dimensions = false`; default is `true` |
...
| `--exclude-selectors SELECTORS` | `exclude_selectors`          | comma-separated CSS selector list                          |
...
| `--tier-strategy STRATEGY`      | `tier_strategy`              | `"auto"` \| `"tier2"`                                      |
