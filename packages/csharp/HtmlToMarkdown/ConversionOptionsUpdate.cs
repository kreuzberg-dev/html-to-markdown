...
///
/// Uses `Option&lt;T&gt;` fields for selective updates. Bindings use this to
/// construct options from language-native types. Prefer
...
public sealed record ConversionOptionsUpdate {
  ...
  public PreprocessingOptionsUpdate
      ? Preprocessing {
          get; init;
        } = null;
  ...
  public static ConversionOptionsUpdate FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<ConversionOptionsUpdate>(json,
                                                                 JsonOptions) ??
             throw new HtmlToMarkdownRsException(
                 $"Failed to parse ConversionOptionsUpdate from JSON: deserializer returned null");
    } catch (HtmlToMarkdownRsException) {
      throw;
    } catch (Exception e) {
      throw new HtmlToMarkdownRsException(
          $"Failed to parse ConversionOptionsUpdate from JSON: {e.Message}", e);
    }
  }
