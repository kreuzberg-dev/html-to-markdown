// This file is auto-generated by alef. DO NOT EDIT.
// alef:hash:f0f83a7f0ec95e9fa6c14023596214a907e0d582a13cd8979fb46cd5f72db15c
#nullable enable

using System;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace HtmlToMarkdown;
/// <summary>
/// Node type enumeration covering all HTML element types.
///
/// This enum categorizes all HTML elements that the converter recognizes,
/// providing a coarse-grained classification for visitor dispatch.
/// </summary>
[JsonConverter(typeof(NodeTypeJsonConverter))]
public enum NodeType {
  /// <summary>
  /// Text node (most frequent - 100+ per document)
  /// </summary>
  [JsonPropertyName("Text")] Text,
  /// <summary>
  /// Generic element node
  /// </summary>
  [JsonPropertyName("Element")] Element,
  /// <summary>
  /// Heading elements (h1-h6)
  /// </summary>
  [JsonPropertyName("Heading")] Heading,
  /// <summary>
  /// Paragraph element
  /// </summary>
  [JsonPropertyName("Paragraph")] Paragraph,
  /// <summary>
  /// Generic div container
  /// </summary>
  [JsonPropertyName("Div")] Div,
  /// <summary>
  /// Blockquote element
  /// </summary>
  [JsonPropertyName("Blockquote")] Blockquote,
  /// <summary>
  /// Preformatted text block
  /// </summary>
  [JsonPropertyName("Pre")] Pre,
  /// <summary>
  /// Horizontal rule
  /// </summary>
  [JsonPropertyName("Hr")] Hr,
  /// <summary>
  /// Ordered or unordered list (ul, ol)
  /// </summary>
  [JsonPropertyName("List")] List,
  /// <summary>
  /// List item (li)
  /// </summary>
  [JsonPropertyName("ListItem")] ListItem,
  /// <summary>
  /// Definition list (dl)
  /// </summary>
  [JsonPropertyName("DefinitionList")] DefinitionList,
  /// <summary>
  /// Definition term (dt)
  /// </summary>
  [JsonPropertyName("DefinitionTerm")] DefinitionTerm,
  /// <summary>
  /// Definition description (dd)
  /// </summary>
  [JsonPropertyName("DefinitionDescription")] DefinitionDescription,
  /// <summary>
  /// Table element
  /// </summary>
  [JsonPropertyName("Table")] Table,
  /// <summary>
  /// Table row (tr)
  /// </summary>
  [JsonPropertyName("TableRow")] TableRow,
  /// <summary>
  /// Table cell (td, th)
  /// </summary>
  [JsonPropertyName("TableCell")] TableCell,
  /// <summary>
  /// Table header cell (th)
  /// </summary>
  [JsonPropertyName("TableHeader")] TableHeader,
  /// <summary>
  /// Table body (tbody)
  /// </summary>
  [JsonPropertyName("TableBody")] TableBody,
  /// <summary>
  /// Table head (thead)
  /// </summary>
  [JsonPropertyName("TableHead")] TableHead,
  /// <summary>
  /// Table foot (tfoot)
  /// </summary>
  [JsonPropertyName("TableFoot")] TableFoot,
  /// <summary>
  /// Anchor link (a)
  /// </summary>
  [JsonPropertyName("Link")] Link,
  /// <summary>
  /// Image (img)
  /// </summary>
  [JsonPropertyName("Image")] Image,
  /// <summary>
  /// Strong/bold (strong, b)
  /// </summary>
  [JsonPropertyName("Strong")] Strong,
  /// <summary>
  /// Emphasis/italic (em, i)
  /// </summary>
  [JsonPropertyName("Em")] Em,
  /// <summary>
  /// Inline code (code)
  /// </summary>
  [JsonPropertyName("Code")] Code,
  /// <summary>
  /// Strikethrough (s, del, strike)
  /// </summary>
  [JsonPropertyName("Strikethrough")] Strikethrough,
  /// <summary>
  /// Underline (u, ins)
  /// </summary>
  [JsonPropertyName("Underline")] Underline,
  /// <summary>
  /// Subscript (sub)
  /// </summary>
  [JsonPropertyName("Subscript")] Subscript,
  /// <summary>
  /// Superscript (sup)
  /// </summary>
  [JsonPropertyName("Superscript")] Superscript,
  /// <summary>
  /// Mark/highlight (mark)
  /// </summary>
  [JsonPropertyName("Mark")] Mark,
  /// <summary>
  /// Small text (small)
  /// </summary>
  [JsonPropertyName("Small")] Small,
  /// <summary>
  /// Line break (br)
  /// </summary>
  [JsonPropertyName("Br")] Br,
  /// <summary>
  /// Span element
  /// </summary>
  [JsonPropertyName("Span")] Span,
  /// <summary>
  /// Article element
  /// </summary>
  [JsonPropertyName("Article")] Article,
  /// <summary>
  /// Section element
  /// </summary>
  [JsonPropertyName("Section")] Section,
  /// <summary>
  /// Navigation element
  /// </summary>
  [JsonPropertyName("Nav")] Nav,
  /// <summary>
  /// Aside element
  /// </summary>
  [JsonPropertyName("Aside")] Aside,
  /// <summary>
  /// Header element
  /// </summary>
  [JsonPropertyName("Header")] Header,
  /// <summary>
  /// Footer element
  /// </summary>
  [JsonPropertyName("Footer")] Footer,
  /// <summary>
  /// Main element
  /// </summary>
  [JsonPropertyName("Main")] Main,
  /// <summary>
  /// Figure element
  /// </summary>
  [JsonPropertyName("Figure")] Figure,
  /// <summary>
  /// Figure caption
  /// </summary>
  [JsonPropertyName("Figcaption")] Figcaption,
  /// <summary>
  /// Time element
  /// </summary>
  [JsonPropertyName("Time")] Time,
  /// <summary>
  /// Details element
  /// </summary>
  [JsonPropertyName("Details")] Details,
  /// <summary>
  /// Summary element
  /// </summary>
  [JsonPropertyName("Summary")] Summary,
  /// <summary>
  /// Form element
  /// </summary>
  [JsonPropertyName("Form")] Form,
  /// <summary>
  /// Input element
  /// </summary>
  [JsonPropertyName("Input")] Input,
  /// <summary>
  /// Select element
  /// </summary>
  [JsonPropertyName("Select")] Select,
  /// <summary>
  /// Option element
  /// </summary>
  [JsonPropertyName("Option")] Option,
  /// <summary>
  /// Button element
  /// </summary>
  [JsonPropertyName("Button")] Button,
  /// <summary>
  /// Textarea element
  /// </summary>
  [JsonPropertyName("Textarea")] Textarea,
  /// <summary>
  /// Label element
  /// </summary>
  [JsonPropertyName("Label")] Label,
  /// <summary>
  /// Fieldset element
  /// </summary>
  [JsonPropertyName("Fieldset")] Fieldset,
  /// <summary>
  /// Legend element
  /// </summary>
  [JsonPropertyName("Legend")] Legend,
  /// <summary>
  /// Audio element
  /// </summary>
  [JsonPropertyName("Audio")] Audio,
  /// <summary>
  /// Video element
  /// </summary>
  [JsonPropertyName("Video")] Video,
  /// <summary>
  /// Picture element
  /// </summary>
  [JsonPropertyName("Picture")] Picture,
  /// <summary>
  /// Source element
  /// </summary>
  [JsonPropertyName("Source")] Source,
  /// <summary>
  /// Iframe element
  /// </summary>
  [JsonPropertyName("Iframe")] Iframe,
  /// <summary>
  /// SVG element
  /// </summary>
  [JsonPropertyName("Svg")] Svg,
  /// <summary>
  /// Canvas element
  /// </summary>
  [JsonPropertyName("Canvas")] Canvas,
  /// <summary>
  /// Ruby annotation
  /// </summary>
  [JsonPropertyName("Ruby")] Ruby,
  /// <summary>
  /// Ruby text
  /// </summary>
  [JsonPropertyName("Rt")] Rt,
  /// <summary>
  /// Ruby parenthesis
  /// </summary>
  [JsonPropertyName("Rp")] Rp,
  /// <summary>
  /// Abbreviation
  /// </summary>
  [JsonPropertyName("Abbr")] Abbr,
  /// <summary>
  /// Keyboard input
  /// </summary>
  [JsonPropertyName("Kbd")] Kbd,
  /// <summary>
  /// Sample output
  /// </summary>
  [JsonPropertyName("Samp")] Samp,
  /// <summary>
  /// Variable
  /// </summary>
  [JsonPropertyName("Var")] Var,
  /// <summary>
  /// Citation
  /// </summary>
  [JsonPropertyName("Cite")] Cite,
  /// <summary>
  /// Quote
  /// </summary>
  [JsonPropertyName("Q")] Q,
  /// <summary>
  /// Deleted text
  /// </summary>
  [JsonPropertyName("Del")] Del,
  /// <summary>
  /// Inserted text
  /// </summary>
  [JsonPropertyName("Ins")] Ins,
  /// <summary>
  /// Data element
  /// </summary>
  [JsonPropertyName("Data")] Data,
  /// <summary>
  /// Meter element
  /// </summary>
  [JsonPropertyName("Meter")] Meter,
  /// <summary>
  /// Progress element
  /// </summary>
  [JsonPropertyName("Progress")] Progress,
  /// <summary>
  /// Output element
  /// </summary>
  [JsonPropertyName("Output")] Output,
  /// <summary>
  /// Template element
  /// </summary>
  [JsonPropertyName("Template")] Template,
  /// <summary>
  /// Slot element
  /// </summary>
  [JsonPropertyName("Slot")] Slot,
  /// <summary>
  /// HTML root element
  /// </summary>
  [JsonPropertyName("Html")] Html,
  /// <summary>
  /// Head element
  /// </summary>
  [JsonPropertyName("Head")] Head,
  /// <summary>
  /// Body element
  /// </summary>
  [JsonPropertyName("Body")] Body,
  /// <summary>
  /// Title element
  /// </summary>
  [JsonPropertyName("Title")] Title,
  /// <summary>
  /// Meta element
  /// </summary>
  [JsonPropertyName("Meta")] Meta,
  /// <summary>
  /// Link element (not anchor)
  /// </summary>
  [JsonPropertyName("LinkTag")] LinkTag,
  /// <summary>
  /// Style element
  /// </summary>
  [JsonPropertyName("Style")] Style,
  /// <summary>
  /// Script element
  /// </summary>
  [JsonPropertyName("Script")] Script,
  /// <summary>
  /// Base element
  /// </summary>
  [JsonPropertyName("Base")] Base,
  /// <summary>
  /// Custom element (web components) or unknown tag
  /// </summary>
  [JsonPropertyName("Custom")] Custom,
}

/// <summary>
/// Custom JSON converter for <see cref="NodeType"/> that respects explicit
/// variant names.
/// </summary>
internal sealed class NodeTypeJsonConverter : JsonConverter<NodeType> {
  public override NodeType Read(ref Utf8JsonReader reader, Type typeToConvert,
                                JsonSerializerOptions options) {
    var value = reader.GetString();
    return value switch {
      "Text" => NodeType.Text,
      "Element" => NodeType.Element,
      "Heading" => NodeType.Heading,
      "Paragraph" => NodeType.Paragraph,
      "Div" => NodeType.Div,
      "Blockquote" => NodeType.Blockquote,
      "Pre" => NodeType.Pre,
      "Hr" => NodeType.Hr,
      "List" => NodeType.List,
      "ListItem" => NodeType.ListItem,
      "DefinitionList" => NodeType.DefinitionList,
      "DefinitionTerm" => NodeType.DefinitionTerm,
      "DefinitionDescription" => NodeType.DefinitionDescription,
      "Table" => NodeType.Table,
      "TableRow" => NodeType.TableRow,
      "TableCell" => NodeType.TableCell,
      "TableHeader" => NodeType.TableHeader,
      "TableBody" => NodeType.TableBody,
      "TableHead" => NodeType.TableHead,
      "TableFoot" => NodeType.TableFoot,
      "Link" => NodeType.Link,
      "Image" => NodeType.Image,
      "Strong" => NodeType.Strong,
      "Em" => NodeType.Em,
      "Code" => NodeType.Code,
      "Strikethrough" => NodeType.Strikethrough,
      "Underline" => NodeType.Underline,
      "Subscript" => NodeType.Subscript,
      "Superscript" => NodeType.Superscript,
      "Mark" => NodeType.Mark,
      "Small" => NodeType.Small,
      "Br" => NodeType.Br,
      "Span" => NodeType.Span,
      "Article" => NodeType.Article,
      "Section" => NodeType.Section,
      "Nav" => NodeType.Nav,
      "Aside" => NodeType.Aside,
      "Header" => NodeType.Header,
      "Footer" => NodeType.Footer,
      "Main" => NodeType.Main,
      "Figure" => NodeType.Figure,
      "Figcaption" => NodeType.Figcaption,
      "Time" => NodeType.Time,
      "Details" => NodeType.Details,
      "Summary" => NodeType.Summary,
      "Form" => NodeType.Form,
      "Input" => NodeType.Input,
      "Select" => NodeType.Select,
      "Option" => NodeType.Option,
      "Button" => NodeType.Button,
      "Textarea" => NodeType.Textarea,
      "Label" => NodeType.Label,
      "Fieldset" => NodeType.Fieldset,
      "Legend" => NodeType.Legend,
      "Audio" => NodeType.Audio,
      "Video" => NodeType.Video,
      "Picture" => NodeType.Picture,
      "Source" => NodeType.Source,
      "Iframe" => NodeType.Iframe,
      "Svg" => NodeType.Svg,
      "Canvas" => NodeType.Canvas,
      "Ruby" => NodeType.Ruby,
      "Rt" => NodeType.Rt,
      "Rp" => NodeType.Rp,
      "Abbr" => NodeType.Abbr,
      "Kbd" => NodeType.Kbd,
      "Samp" => NodeType.Samp,
      "Var" => NodeType.Var,
      "Cite" => NodeType.Cite,
      "Q" => NodeType.Q,
      "Del" => NodeType.Del,
      "Ins" => NodeType.Ins,
      "Data" => NodeType.Data,
      "Meter" => NodeType.Meter,
      "Progress" => NodeType.Progress,
      "Output" => NodeType.Output,
      "Template" => NodeType.Template,
      "Slot" => NodeType.Slot,
      "Html" => NodeType.Html,
      "Head" => NodeType.Head,
      "Body" => NodeType.Body,
      "Title" => NodeType.Title,
      "Meta" => NodeType.Meta,
      "LinkTag" => NodeType.LinkTag,
      "Style" => NodeType.Style,
      "Script" => NodeType.Script,
      "Base" => NodeType.Base,
      "Custom" => NodeType.Custom,
      _ => throw new JsonException($"Unknown NodeType value: {value}")
    };
  }

  public override void Write(Utf8JsonWriter writer, NodeType value,
                             JsonSerializerOptions options) {
    var str = value switch {
      NodeType.Text => "Text",
      NodeType.Element => "Element",
      NodeType.Heading => "Heading",
      NodeType.Paragraph => "Paragraph",
      NodeType.Div => "Div",
      NodeType.Blockquote => "Blockquote",
      NodeType.Pre => "Pre",
      NodeType.Hr => "Hr",
      NodeType.List => "List",
      NodeType.ListItem => "ListItem",
      NodeType.DefinitionList => "DefinitionList",
      NodeType.DefinitionTerm => "DefinitionTerm",
      NodeType.DefinitionDescription => "DefinitionDescription",
      NodeType.Table => "Table",
      NodeType.TableRow => "TableRow",
      NodeType.TableCell => "TableCell",
      NodeType.TableHeader => "TableHeader",
      NodeType.TableBody => "TableBody",
      NodeType.TableHead => "TableHead",
      NodeType.TableFoot => "TableFoot",
      NodeType.Link => "Link",
      NodeType.Image => "Image",
      NodeType.Strong => "Strong",
      NodeType.Em => "Em",
      NodeType.Code => "Code",
      NodeType.Strikethrough => "Strikethrough",
      NodeType.Underline => "Underline",
      NodeType.Subscript => "Subscript",
      NodeType.Superscript => "Superscript",
      NodeType.Mark => "Mark",
      NodeType.Small => "Small",
      NodeType.Br => "Br",
      NodeType.Span => "Span",
      NodeType.Article => "Article",
      NodeType.Section => "Section",
      NodeType.Nav => "Nav",
      NodeType.Aside => "Aside",
      NodeType.Header => "Header",
      NodeType.Footer => "Footer",
      NodeType.Main => "Main",
      NodeType.Figure => "Figure",
      NodeType.Figcaption => "Figcaption",
      NodeType.Time => "Time",
      NodeType.Details => "Details",
      NodeType.Summary => "Summary",
      NodeType.Form => "Form",
      NodeType.Input => "Input",
      NodeType.Select => "Select",
      NodeType.Option => "Option",
      NodeType.Button => "Button",
      NodeType.Textarea => "Textarea",
      NodeType.Label => "Label",
      NodeType.Fieldset => "Fieldset",
      NodeType.Legend => "Legend",
      NodeType.Audio => "Audio",
      NodeType.Video => "Video",
      NodeType.Picture => "Picture",
      NodeType.Source => "Source",
      NodeType.Iframe => "Iframe",
      NodeType.Svg => "Svg",
      NodeType.Canvas => "Canvas",
      NodeType.Ruby => "Ruby",
      NodeType.Rt => "Rt",
      NodeType.Rp => "Rp",
      NodeType.Abbr => "Abbr",
      NodeType.Kbd => "Kbd",
      NodeType.Samp => "Samp",
      NodeType.Var => "Var",
      NodeType.Cite => "Cite",
      NodeType.Q => "Q",
      NodeType.Del => "Del",
      NodeType.Ins => "Ins",
      NodeType.Data => "Data",
      NodeType.Meter => "Meter",
      NodeType.Progress => "Progress",
      NodeType.Output => "Output",
      NodeType.Template => "Template",
      NodeType.Slot => "Slot",
      NodeType.Html => "Html",
      NodeType.Head => "Head",
      NodeType.Body => "Body",
      NodeType.Title => "Title",
      NodeType.Meta => "Meta",
      NodeType.LinkTag => "LinkTag",
      NodeType.Style => "Style",
      NodeType.Script => "Script",
      NodeType.Base => "Base",
      NodeType.Custom => "Custom",
      _ => throw new JsonException($"Unknown NodeType value: {value}")
    };
    writer.WriteStringValue(str);
  }
}
