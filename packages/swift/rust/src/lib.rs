...
/// the underlying ARC reference for the lifetime of this struct.
///
/// Send + Sync are asserted unsafely: Swift classes used as trait bridges must be thread-safe.
pub struct SwiftHtmlVisitorWrapper {
    inner: ffi::SwiftHtmlVisitorBox,
}
// SAFETY: ~keep The Swift API requires conformers to synchronize all shared mutable
// state.
unsafe impl Send for SwiftHtmlVisitorWrapper {}
// SAFETY: ~keep The same documented synchronization contract permits
// shared references.
