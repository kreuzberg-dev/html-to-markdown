...
// `crates/html-to-markdown-rs-swift` and the FFI crate both build `crate-type = ["cdylib", "staticlib"]`,
...
let package = Package(
  ...
  targets: [
    ...
    .target(
      ...
      linkerSettings: [
        .unsafeFlags([
          resolvedStaticLib("html_to_markdown_rs_swift"),
          resolvedStaticLib("html_to_markdown_ffi"),
        ]),
