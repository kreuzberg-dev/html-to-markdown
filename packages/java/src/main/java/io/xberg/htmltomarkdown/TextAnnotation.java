// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:f0f83a7f0ec95e9fa6c14023596214a907e0d582a13cd8979fb46cd5f72db15c
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
package io.xberg.htmltomarkdown;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import com.fasterxml.jackson.databind.annotation.JsonPOJOBuilder;

/**
 * A styling or semantic annotation that applies to a byte range within a node's
 * text.
 *
 * Unlike DocumentNode, which captures block-level structure (headings,
 * paragraphs, etc.), a {@code TextAnnotation} describes inline-level markup —
 * bold, italic, links, code spans, and similar — that spans a contiguous run of
 * bytes inside {@code DocumentNode.content}'s text field.
 *
 * Byte offsets ({@code start}..{@code end}) are into the UTF-8 encoded text of
 * the parent node. The range is half-open: {@code start} is inclusive and
 * {@code end} is exclusive.
 *
 * Multiple annotations on the same node can overlap (e.g. bold-italic text),
 * and they are stored in the order they are encountered during DOM traversal.
 *
 * See AnnotationKind for the full list of supported annotation types.
 */
@JsonInclude(JsonInclude.Include.NON_ABSENT)
@JsonDeserialize(builder = TextAnnotation.Builder.class)
public record TextAnnotation(@JsonProperty("start") int start,
                             @JsonProperty("end") int end,
                             @JsonProperty("kind") AnnotationKind kind) {
  /** Creates a new Builder for constructing instances of this record. */
  public static Builder builder() { return new Builder(); }

  // CPD-OFF
  /** Jackson builder for TextAnnotation deserialization. */
  @com.fasterxml.jackson.annotation.JsonIgnoreProperties(ignoreUnknown = true)
  @JsonPOJOBuilder(withPrefix = "with", buildMethodName = "build")
  public static final class Builder {

    private int start;
    private int end;
    private AnnotationKind kind;

    /** Sets the start field. */
    @JsonProperty("start")
    public Builder withStart(final int value) {
      this.start = value;
      return this;
    }

    /** Sets the end field. */
    @JsonProperty("end")
    public Builder withEnd(final int value) {
      this.end = value;
      return this;
    }

    /** Sets the kind field. */
    @JsonProperty("kind")
    public Builder withKind(final AnnotationKind value) {
      this.kind = value;
      return this;
    }

    /**
     * Constructs a TextAnnotation instance from the builder's current state.
     */
    public TextAnnotation build() {
      return new TextAnnotation(start, end, kind);
    }
  }
  // CPD-ON
}
