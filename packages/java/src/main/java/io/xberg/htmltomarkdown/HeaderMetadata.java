// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:94dd1be9921f0c99181696abdb7a25b59df4f9229ed5f05876faf9831bbba0f5
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
package io.xberg.htmltomarkdown;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import com.fasterxml.jackson.databind.annotation.JsonPOJOBuilder;
import org.jspecify.annotations.Nullable;

/**
 * Header element metadata with hierarchy tracking.
 *
 * Captures heading elements (h1-h6) with their text content, identifiers,
 * and position in the document structure.
 */
@JsonInclude(JsonInclude.Include.NON_ABSENT)
@JsonDeserialize(builder = HeaderMetadata.Builder.class)
public record HeaderMetadata(@JsonProperty("level") byte level,
                             @JsonProperty("text") String text,
                             @Nullable @JsonProperty("id") String id,
                             @JsonProperty("depth") long depth,
                             @JsonProperty("html_offset") long htmlOffset) {
  /** Creates a new Builder for constructing instances of this record. */
  public static Builder builder() { return new Builder(); }

  // CPD-OFF
  /** Jackson builder for HeaderMetadata deserialization. */
  @com.fasterxml.jackson.annotation.JsonIgnoreProperties(ignoreUnknown = true)
  @JsonPOJOBuilder(withPrefix = "with", buildMethodName = "build")
  public static final class Builder {

    private byte level;
    private String text;
    @Nullable private String id;
    private long depth;
    @JsonProperty("html_offset") private long htmlOffset;

    /** Sets the level field. */
    @JsonProperty("level")
    public Builder withLevel(final byte value) {
      this.level = value;
      return this;
    }

    /** Sets the text field. */
    @JsonProperty("text")
    public Builder withText(final String value) {
      this.text = value;
      return this;
    }

    /** Sets the id field. */
    @JsonProperty("id")
    public Builder withId(final @Nullable String value) {
      this.id = value;
      return this;
    }

    /** Sets the depth field. */
    @JsonProperty("depth")
    public Builder withDepth(final long value) {
      this.depth = value;
      return this;
    }

    /** Sets the htmlOffset field. */
    @JsonProperty("html_offset")
    public Builder withHtmlOffset(final long value) {
      this.htmlOffset = value;
      return this;
    }

    /**
     * Constructs a HeaderMetadata instance from the builder's current state.
     */
    public HeaderMetadata build() {
      return new HeaderMetadata(level, text, id, depth, htmlOffset);
    }
  }
  // CPD-ON
}
