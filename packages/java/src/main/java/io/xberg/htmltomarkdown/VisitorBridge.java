...
// alef:hash:825e0e3ff195402624a89e4fdc61aa6f2ba47a114c36a820a5101c9527367168
...
final class VisitorBridge implements AutoCloseable {
    ...
    // Callback table: user_data + free_string + 40 callbacks
    // = 42 pointer-sized slots
    private static final long CALLBACKS_STRUCT_SIZE =
        (long) ValueLayout.ADDRESS.byteSize() * 42L;

    // `NodeContext` exactly as the FFI backend lays it out in C:
    ...
    private static final long CTX_OFFSET_NODE_TYPE =
        CTX_LAYOUT.byteOffset(MemoryLayout.PathElement.groupElement("node_type"));
    private static final long CTX_OFFSET_TAG_NAME =
        CTX_LAYOUT.byteOffset(MemoryLayout.PathElement.groupElement("tag_name"));
    private static final long CTX_OFFSET_DEPTH =
        CTX_LAYOUT.byteOffset(MemoryLayout.PathElement.groupElement("depth"));
    private static final long CTX_OFFSET_INDEX_IN_PARENT =
        CTX_LAYOUT.byteOffset(MemoryLayout.PathElement.groupElement("index_in_parent"));
    private static final long CTX_OFFSET_PARENT_TAG =
        CTX_LAYOUT.byteOffset(MemoryLayout.PathElement.groupElement("parent_tag"));
    private static final long CTX_OFFSET_IS_INLINE =
        CTX_LAYOUT.byteOffset(MemoryLayout.PathElement.groupElement("is_inline"));
    private final Arena arena;
    private final MemorySegment struct;
    private final HtmlVisitor visitor;
    private final ConcurrentHashMap<Long, Arena> callbackAllocations = new ConcurrentHashMap<>();
    ...
    VisitorBridge(HtmlVisitor visitor) {
        ...
        try {
            ...
            var freeHandle = LOOKUP.findVirtual(
                VisitorBridge.class,
                "freeCallbackString",
                MethodType.methodType(void.class, MemorySegment.class)).bindTo(this);
            var freeStub = LINKER.upcallStub(
                freeHandle,
                FunctionDescriptor.ofVoid(ValueLayout.ADDRESS),
                arena);
            struct.set(ValueLayout.ADDRESS, offset, freeStub);
    ...
    private void freeCallbackString(MemorySegment value) {
        var allocation = callbackAllocations.remove(value.address());
        if (allocation != null) {
            allocation.close();
        }
    }
    ...
    private static NodeContext decodeContext(
        ...
        return new NodeContext(
            NodeType.values()[ctx.get(ValueLayout.JAVA_INT, CTX_OFFSET_NODE_TYPE)],
            ctx.get(ValueLayout.ADDRESS, CTX_OFFSET_TAG_NAME)
                .reinterpret(Long.MAX_VALUE).getString(0),
            ctx.get(ValueLayout.JAVA_LONG, CTX_OFFSET_DEPTH),
            ctx.get(ValueLayout.JAVA_LONG, CTX_OFFSET_INDEX_IN_PARENT),
            ctx.get(ValueLayout.ADDRESS, CTX_OFFSET_PARENT_TAG).equals(MemorySegment.NULL)
                ? null
                : ctx.get(ValueLayout.ADDRESS, CTX_OFFSET_PARENT_TAG)
                    .reinterpret(Long.MAX_VALUE).getString(0),
            ctx.get(ValueLayout.JAVA_INT, CTX_OFFSET_IS_INLINE) != 0
        );
    }
    ...
    private int encodeVisitResult(VisitResult result, MemorySegment outCustom, MemorySegment outLen) {
        return switch (result) {
            ...
            case VisitResult.Custom c -> {
                var allocation = Arena.ofShared();
                var buf = allocation.allocateFrom(c.value());
                callbackAllocations.put(buf.address(), allocation);
    ...
    @Override
    public void close() {
        callbackAllocations.values().forEach(Arena::close);
        callbackAllocations.clear();
        arena.close();
    }
}
