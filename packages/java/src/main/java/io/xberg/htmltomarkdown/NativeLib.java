...
final class NativeLib {
  ...
  private static Path loadNativeLibrary() {
    ...
    try {
      ...
      String msg =
          "Failed to load html_to_markdown_ffi native library. Expected "
          + "resource: " + nativesDir + "/" + libName + libExt +
          " (RID: " + nativesRid + "). " +
  ...
  private static Path tryExtractAndLoadFromResources(String nativesDir,
    ...
    try {
      ...
      System.err.println("[NativeLib] Failed to extract and load native "
                         + "library from resources: " + e.getMessage());
      return null;
    }
  }
  ...
  private static Path extractOrReuseNativeDirectory(String nativesDir)
    ...
    if (location == null) {
      throw new IllegalStateException(
          "Missing code source location for html_to_markdown_ffi JAR");
    }
  ...
  static final MethodHandle HTM_FREE_BYTES = LINKER.downcallHandle(
      LIB.find("htm_free_bytes")
          .or(() -> LIB.find("_htm_free_bytes")) // Try underscore-prefixed
          // variant for macOS
          .or(()
                  -> LINKER.defaultLookup().find(
                      "htm_free_bytes")) // Fallback to default lookup
          .or(()
                  -> LINKER.defaultLookup().find(
                      "_htm_free_bytes")) // Fallback underscore variant
          .orElseThrow(),
      FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, ValueLayout.JAVA_LONG,
                                ValueLayout.JAVA_LONG));

  static final MethodHandle HTM_LAST_ERROR_CODE =
      LINKER.downcallHandle(LIB.find("htm_last_error_code").orElse(null),
                            FunctionDescriptor.of(ValueLayout.JAVA_LONG));

  static final MethodHandle HTM_LAST_ERROR_CONTEXT =
      LINKER.downcallHandle(LIB.find("htm_last_error_context").orElse(null),
                            FunctionDescriptor.of(ValueLayout.ADDRESS));

  static final MethodHandle HTM_CONVERSION_RESULT_TO_JSON =
      LIB.find("htm_conversion_result_to_json")
          .map(s
  ...
  static final MethodHandle HTM_CONVERSION_OPTIONS_FROM_JSON =
      LINKER.downcallHandle(
          LIB.find("htm_conversion_options_from_json")
              ...
              .or(()
                      -> LINKER.defaultLookup().find(
                          "_htm_conversion_options_from_json"))
              // Fallback underscore variant
              .orElseThrow(),
          FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.ADDRESS));
  ...
  static final MethodHandle HTM_REGISTER_HTML_VISITOR =
      LIB.find("htm_register_html_visitor")
          .map(s
               -> LINKER.downcallHandle(
  ...
  static final MethodHandle HTM_VISITOR_CREATE = LINKER.downcallHandle(
      LIB.find("htm_visitor_create")
          .or(() -> LIB.find("_htm_visitor_create")) // Try underscore-prefixed
          // variant for macOS
          .orElseThrow(),
      FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.ADDRESS));
  ...
  static final MethodHandle HTM_VISITOR_FREE = LINKER.downcallHandle(
      LIB.find("htm_visitor_free")
          .or(() -> LIB.find("_htm_visitor_free")) // Try underscore-prefixed
          // variant for macOS
          .orElseThrow(),
      FunctionDescriptor.ofVoid(ValueLayout.ADDRESS));
  ...
  static final MethodHandle HTM_OPTIONS_SET_VISITOR = LINKER.downcallHandle(
      LIB.find("htm_options_set_visitor")
          .or(()
                  -> LIB.find(
                      "_htm_options_set_visitor")) // Try underscore-prefixed
          // variant for macOS
          .orElseThrow(),
