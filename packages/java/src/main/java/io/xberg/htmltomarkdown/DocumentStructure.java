// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:11b53512ce352fa429b3487113320a2a4a72d302f7a5f4f2ceda41991f6d1847
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
package io.xberg.htmltomarkdown;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import com.fasterxml.jackson.databind.annotation.JsonPOJOBuilder;
import java.util.List;
import org.jspecify.annotations.Nullable;

/**
 * A structured document tree representing the semantic content of an HTML
 * document.
 *
 * Uses a flat node array with index-based parent/child references for efficient
 * traversal.
 */
@JsonInclude(JsonInclude.Include.NON_ABSENT)
@JsonDeserialize(builder = DocumentStructure.Builder.class)
public record DocumentStructure(@JsonProperty("nodes") List<DocumentNode> nodes,
                                @Nullable @JsonProperty("source_format")
                                String sourceFormat) {
  /** Creates a new Builder for constructing instances of this record. */
  public static Builder builder() { return new Builder(); }

  // CPD-OFF
  /** Jackson builder for DocumentStructure deserialization. */
  @com.fasterxml.jackson.annotation.JsonIgnoreProperties(ignoreUnknown = true)
  @JsonPOJOBuilder(withPrefix = "with", buildMethodName = "build")
  public static final class Builder {

    private List<DocumentNode> nodes = List.of();
    @JsonProperty("source_format") @Nullable private String sourceFormat;

    /** Sets the nodes field. */
    @JsonProperty("nodes")
    public Builder withNodes(final List<DocumentNode> value) {
      this.nodes = value;
      return this;
    }

    /** Sets the sourceFormat field. */
    @JsonProperty("source_format")
    public Builder withSourceFormat(final @Nullable String value) {
      this.sourceFormat = value;
      return this;
    }

    /**
     * Constructs a DocumentStructure instance from the builder's current
     * state.
     */
    public DocumentStructure build() {
      return new DocumentStructure(nodes, sourceFormat);
    }
  }
  // CPD-ON
}
