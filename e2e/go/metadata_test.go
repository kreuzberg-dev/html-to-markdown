...
// alef:hash:8e8d0de141d4b0c76781b55ce4c06ba119eb52d7cd34d6ed32cf934fa0c940fe
...
func Test_OgBasicTags(t *testing.T) {
	...
	if string(metadataDocumentOpenGraphTitle) != `OG Title` {
		t.Errorf("equals mismatch: got %v", metadataDocumentOpenGraphTitle)
	}
	if string(metadataDocumentOpenGraphDescription) != `OG description text.` {
		t.Errorf("equals mismatch: got %v", metadataDocumentOpenGraphDescription)
	}
	if string(metadataDocumentOpenGraphImage) != `https://example.com/image.jpg` {
...
func Test_OgMultipleTags(t *testing.T) {
	...
	if string(metadataDocumentOpenGraphTitle) != `Article Title` {
		t.Errorf("equals mismatch: got %v", metadataDocumentOpenGraphTitle)
	}
	if string(metadataDocumentOpenGraphType) != `article` {
		t.Errorf("equals mismatch: got %v", metadataDocumentOpenGraphType)
	}
	if string(metadataDocumentOpenGraphURL) != `https://example.com/article` {
		t.Errorf("equals mismatch: got %v", metadataDocumentOpenGraphURL)
	}
	if string(metadataDocumentOpenGraphSiteName) != `Example Site` {
...
func Test_StructuredDataJsonLd(t *testing.T) {
	...
	if strings.Contains(string(*result.Content), `application/ld+json`) {
		t.Errorf("expected NOT to contain %s, got %v", `application/ld+json`, result.Content)
	}
	if strings.Contains(string(*result.Content), `@context`) {
		t.Errorf("expected NOT to contain %s, got %v", `@context`, result.Content)
	}
	if strings.Contains(string(*result.Content), `schema.org`) {
...
func Test_StructuredDataMultipleJsonLd(t *testing.T) {
	...
	if strings.Contains(string(*result.Content), `BreadcrumbList`) {
		t.Errorf("expected NOT to contain %s, got %v", `BreadcrumbList`, result.Content)
	}
	if strings.Contains(string(*result.Content), `ListItem`) {
...
func Test_TwitterCardTags(t *testing.T) {
	...
	if string(metadataDocumentTwitterCardCard) != `summary_large_image` {
		t.Errorf("equals mismatch: got %v", metadataDocumentTwitterCardCard)
	}
	if string(metadataDocumentTwitterCardTitle) != `Twitter Card Title` {
		t.Errorf("equals mismatch: got %v", metadataDocumentTwitterCardTitle)
	}
	if string(metadataDocumentTwitterCardDescription) != `Twitter card description.` {
