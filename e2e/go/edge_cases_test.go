...
func Test_HiddenContentAriaHiddenStillRendered(t *testing.T) {
	...
	if result.Content != nil {
	if !strings.Contains(string(*result.Content), `visible`) {
		t.Errorf("expected to contain %s", `visible`)
	}
	}
	if result.Content != nil {
	if !strings.Contains(string(*result.Content), `still shown`) {
		t.Errorf("expected to contain %s", `still shown`)
	}
	}
	if result.Content != nil {
	if !strings.Contains(string(*result.Content), `also visible`) {
