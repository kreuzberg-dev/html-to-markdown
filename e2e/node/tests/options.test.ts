...
// alef:hash:7d887685face0d7a74fd82cfd66ed7c4c2741e0ff4d9f23c896ef8a46a5bdbe5
// To regenerate: alef generate
// To verify freshness: alef verify

import { describe, expect, it } from "vitest";
import { convert, type ConversionOptions, CodeBlockStyle, HeadingStyle, HighlightStyle, LinkStyle, ListIndentType, NewlineStyle, OutputFormat, PreprocessingPreset, UrlEscapeStyle, WhitespaceMode } from "@xberg-io/html-to-markdown";
...
describe("options", () => {
	...
	it("issue_396_backticks_blank_line_after_fence: Backticks code block trims trailing newline inside fence and adds blank line after closing fence (issue #396)", () => {
		const result = convert("<p>Foo</p><pre><code>1\n2\n</code></pre><p>Bar</p>", { codeBlockStyle: CodeBlockStyle.Backticks } as ConversionOptions);
	...
	it("options_code_block_backticks: Backticks code block style uses triple backtick fences", () => {
		const result = convert("<pre><code class=\"language-js\">console.log('hi');</code></pre>", { codeBlockStyle: CodeBlockStyle.Backticks } as ConversionOptions);
	...
	it("options_code_block_indented: Code blocks use 4-space indentation", () => {
		const result = convert("<pre><code>print('hello')</code></pre>", { codeBlockStyle: CodeBlockStyle.Indented } as ConversionOptions);
	...
	it("options_code_block_tildes: Code blocks use tilde fences", () => {
		const result = convert("<pre><code>let x = 1;</code></pre>", { codeBlockStyle: CodeBlockStyle.Tildes } as ConversionOptions);
    expect(result.content).toContain("~~~");
	...
	it("options_code_block_tildes_style: Tildes code block style uses triple tilde fences", () => {
		const result = convert("<pre><code>some code</code></pre>", { codeBlockStyle: CodeBlockStyle.Tildes } as ConversionOptions);
    expect(result.content).toContain("~~~");
	...
	it("options_exclude_selectors_plain_text_mode: Exclude selectors work in plain text output mode", () => {
		const result = convert("<body><div class=\"nav\">Navigation</div><p>Article body</p></body>", { excludeSelectors: [".nav"], outputFormat: OutputFormat.Plain } as ConversionOptions);
	...
	it("options_highlight_bold: Mark tag rendered as bold", () => {
		const result = convert("<p>Text with <mark>highlighted</mark> text.</p>", { highlightStyle: HighlightStyle.Bold } as ConversionOptions);
    expect(result.content).toContain("**highlighted**");

	}, 30000);

	it("options_highlight_double_equal: Mark tag with double equal highlight style", () => {
		const result = convert("<p>Text with <mark>highlighted</mark> here.</p>", { highlightStyle: HighlightStyle.DoubleEqual } as ConversionOptions);
    expect(result.content).toContain("==highlighted==");

	}, 30000);

	it("options_highlight_none: Mark tag with no highlight style strips the mark", () => {
		const result = convert("<p>Text with <mark>plain</mark> content.</p>", { highlightStyle: HighlightStyle.None } as ConversionOptions);
	...
	it("options_link_style_reference: Links use reference-style formatting", () => {
		const result = convert("<p><a href='https://example.com'>Example</a> and <a href='https://other.com'>Other</a></p>", { linkStyle: LinkStyle.Reference } as ConversionOptions);
	...
	it("options_list_indent_tabs: Tab indentation type for nested list items", () => {
		const result = convert("<ul><li>Parent<ul><li>Child</li></ul></li></ul>", { listIndentType: ListIndentType.Tabs } as ConversionOptions);
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
	...
	it("options_newline_backslash: Hard line breaks rendered with backslash", () => {
		const result = convert("<p>Line one<br>Line two</p>", { newlineStyle: NewlineStyle.Backslash } as ConversionOptions);
	...
	it("options_output_format_markdown: Default markdown output format produces standard markdown", () => {
		const result = convert("<h1>Title</h1><p>Some text.</p>", { headingStyle: HeadingStyle.Atx, outputFormat: OutputFormat.Markdown } as ConversionOptions);
    expect(result.content).toContain("# Title");
	...
	it("options_output_format_plain: Plain text output format strips markdown syntax", () => {
		const result = convert("<h1>Title</h1><p>Some <strong>bold</strong> text.</p>", { outputFormat: OutputFormat.Plain } as ConversionOptions);
    expect(result.content).toContain("Title");
	...
	it("options_preprocessing_aggressive: Aggressive preset removes nav, footer, aside unconditionally", () => {
		const result = convert("<nav>Menu</nav><article><h1>Title</h1><p>Content</p></article><aside>Sidebar</aside><footer>Footer</footer>", { preprocessing: { preset: PreprocessingPreset.Aggressive } } as ConversionOptions);
    expect(result.content).toContain("Title");
    expect(result.content).toContain("Content");
    expect(result.content).not.toContain("Menu");
	...
	it("options_preprocessing_minimal: Minimal preset preserves nav, footer, aside", () => {
		const result = convert("<nav>Navigation</nav><p>Content</p><footer>Footer</footer>", { preprocessing: { preset: PreprocessingPreset.Minimal } } as ConversionOptions);
	...
	it("options_url_escape_style_angle_default: Default angle style wraps URLs containing spaces in angle brackets", () => {
		const result = convert("<a href=\"/file (1).pdf\">file</a>", { urlEscapeStyle: UrlEscapeStyle.Angle } as ConversionOptions);
	...
	it("options_url_escape_style_percent_angle_brackets_in_url: Percent style encodes angle brackets in link URLs from issue 392", () => {
		const result = convert("<a href=\"/file (1) <draft>.pdf\">file</a>", { urlEscapeStyle: UrlEscapeStyle.Percent } as ConversionOptions);
	...
	it("options_url_escape_style_percent_image: Percent style encodes special characters in image src URLs", () => {
		const result = convert("<img src=\"/img (1) <draft>.png\" alt=\"alt\">", { urlEscapeStyle: UrlEscapeStyle.Percent } as ConversionOptions);
	...
	it("options_url_escape_style_percent_link: Percent style encodes spaces and parens in link URLs", () => {
		const result = convert("<a href=\"/file (1).pdf\">file</a>", { urlEscapeStyle: UrlEscapeStyle.Percent } as ConversionOptions);
	...
	it("options_whitespace_normalized: Normalized whitespace mode collapses multiple spaces", () => {
		const result = convert("<p>Text   with    extra   spaces.</p>", { whitespaceMode: WhitespaceMode.Normalized } as ConversionOptions);
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
	...
	it("options_whitespace_strict: Strict whitespace mode preserves whitespace as-is", () => {
		const result = convert("<p>Preserved   spacing.</p>", { whitespaceMode: WhitespaceMode.Strict } as ConversionOptions);
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
