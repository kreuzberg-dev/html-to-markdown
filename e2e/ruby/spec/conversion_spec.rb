...
# alef:hash:bb2208cd3c1377259369242a7d2a65b457f892fb8071155db434a0554aedb59e
...
RSpec.describe 'conversion' do
  ...
  it 'blockquote_text_then_paragraph_gets_blank_line: Bare text followed by a paragraph inside a blockquote gets a blank-line separator instead of merging into one line' do
  ...
  it 'table_nested_chain_not_misclassified_as_layout: A straight chain of one-nested-table-per-cell tables renders as pipe tables, not a broken layout list' do
  ...
  it 'table_ragged_row_fewer_cells_than_header: A data row with fewer cells than the header is padded to the declared column count' do
  ...
  it 'table_ragged_row_more_cells_than_header: A data row with more cells than the header widens the separator so no cell is silently dropped' do
